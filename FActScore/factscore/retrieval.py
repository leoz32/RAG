...
def _simple_token_chunks(text, max_length):
    tokens = text.split()
    if not tokens:
        return []
    return [tokens[index:index + max_length] for index in range(0, len(tokens), max_length)]
            ...
            for line in f:
                ...
                for sent_idx, sent in enumerate(text):
                    ...
                    if tokenizer is not None:
                        ...
                        word_chunks = _simple_token_chunks(sent, MAX_LENGTH)
                        if not passages[-1]:
                            passages.pop()
                        passages.extend(word_chunks)

                if tokenizer is not None:
                    psgs = [tokenizer.decode(tokens) for tokens in passages if np.sum([t not in [0, 2] for t in tokens])>0]
                else:
                    psgs = [" ".join(tokens).strip() for tokens in passages if tokens]
...
class Retrieval(object):
    ...
    def get_bm25_passages(self, topic, query, passages, k):
        if topic in self.embed_cache:
            ...
            bm25 = BM25Okapi([
                _tokenize_for_bm25(psg["text"].replace("<s>", "").replace("</s>", ""))
                for psg in passages
            ])
